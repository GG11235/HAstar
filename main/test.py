...
class States(object):
    ...
    def estimate_rs_cost(self, sx, sy, syaw, gx, gy, gyaw, ox, oy):
        ...
        isupdated, fpath = planner.update_node_with_analystic_expantion(nstart, ngoal, c, oox, ooy, kdtree, False)
        if isupdated:
            cost = fpath.cost
            return cost
        else:
...
for x in range(-12,12,3):
    for y in range(6,7,3):
        for yaw in range(-30,30,15):
            yield_x.append(x)
            yield_y.append(y)
            yield_yaw.append(yaw)

for x in range(-6,4,2):
...
for i in range(len(states_set)):
    ...
    # plt.grid(True)
    # plt.axis("equal")
    # vehicle.plot_trailer(states.mx, states.my, states.myaw, 0)
    # plt.pause(0.1)
